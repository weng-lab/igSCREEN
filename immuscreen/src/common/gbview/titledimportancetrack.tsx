...
const ImportantRegions: React.FC<ImportantRegionTrackProps> = (props) => {
  const transform = linearTransform(
      ...
      {props.positiveRegions
        ...
        .map((r, i) => (
          <rect
            ...
            fill="#2d4f25"
            onClick={() =>
              props.onRegionClick && props.onRegionClick([r.start, r.end])
            }
          />
        ))}
      {props.negativeRegions
        .filter((r) => r.end - r.start > 3)
        .map((r, i) => (
          <rect
            key={`important_${i}`}
...
console.log("domain",signalURL, props.domain)
  ...
  const onImportantRegionClick = useCallback(
    (coordinates: [number, number]) => {
      fetch("https://ga.staging.wenglab.org/graphql", {
        method: "POST",
        headers: {
  ...
  return (
    <g transform={transform}>
      <EmptyTrack transform="" text={title} height={30} width={1400} id="" />
