...
export default function IcreEQTLs({ accession }: { accession: string }) {
  const { loading, data, error } = useQuery(ICRE_EQTL_QUERY, {
    variables: {
      ccre: [accession],
    },
    skip: !accession,
    ...
    <Stack spacing={2}>
      <Box sx={{ flex: "1 1 auto" }}>
        <DataGridPro
          ...
          slotProps={{ toolbar: { title: `GTEX whole-blood eQTLs for ${accession}` } }}
          pagination
          initialState={{
            sorting: {
      ...
      <Box sx={{ flex: "1 1 auto" }}>
        <DataGridPro
          ...
          slots={{ toolbar: DataGridToolbar }}
          slotProps={{ toolbar: { title: `OneK1K eQTLs for ${accession}` } }}
          pagination
          initialState={{
            sorting: {
