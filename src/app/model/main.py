...
ID_REGEX = re.compile(r"\b([A-Za-z]{1,5}\d{1,7})\b")
...
def load_genome():
    ...
    return ["BRCA1", "TP53", "NF1", "BRCA2", "CFTR", "EGFR"]

def fetch_genome(genome_id):
    #Fetch genome data with matching genome ids
    dummy_db = {
        "BRCA1": {"name":"BRCA1", "description":"Tumor suppressor gene on chr17."},
        "TP53": {"name":"TP53", "description":"Tumor protein p53 on chr17."},
        "NF1": {"name": "NF1", "description": "Cell Growth protein on chr17"},
        "BRCA2": {"name":"BRCA2", "description":"BRCA2"},
        "CFTR": {"name":"CFTR", "description":"CFTR"},
        "EGFR": {"name": "EGFR", "description": "EGFR"},
    }
    return dummy_db.get(genome_id)
...
def main():
    ...
    while True:
        ...
        if portal:
            ...
            if link_record:
                print(f"Assistnat: Here's the {portal}")
